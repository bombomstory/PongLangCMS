<?php
    ...
    $sql2 = "SELECT MAX( orders ) AS max FROM `plcms_performance`;";
    ...
    switch ($do) {
        case "increase_menu_orders":
          ...
          if ($orders != 1){
              $new_orders = $orders-1;
              $sql = "UPDATE plcms_performance SET orders = ".$orders." WHERE orders = ".$new_orders.";";
              mysqli_query($conn, $sql);
              $sql = "UPDATE plcms_performance SET orders = ".$new_orders." WHERE id=".$id.";";
              mysqli_query($conn, $sql);
          }
          break;
...
<p>
<form class="form-horizontal" method="POST" action="performance-manage.php?do=save_add_menu">
...
<?php
        ...
        case "save_add_menu":
          ...
          $sql = "SELECT max(orders) as max_orders FROM plcms_performance";
          $result = mysqli_query($conn, $sql);
          $row = mysqli_fetch_assoc($result);
          $max_orders = $row["max_orders"]+1;

          $sql = "INSERT INTO `plcms_performance` (`id`, `menuname`, `link`, `orders`) VALUES (NULL, \"$menuname\", \"$link\", ".$max_orders.");";
          mysqli_query($conn, $sql);

          break;
        case "delete_menu":
            $sql = "DELETE FROM `plcms_performance` WHERE `plcms_performance`.`id` = ".$id.";";
            mysqli_query($conn, $sql);
            break;
        case "edit_menu":
            $sql = "SELECT * FROM `plcms_performance` WHERE `plcms_performance`.`id` = ".$id.";";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_assoc($result);
?>

<p>
<form class="form-horizontal" method="POST" action="performance-manage.php?do=save_edit_menu">
  <div class="form-group">
...
<?php
        ...
        case "save_edit_menu": 
            ...
            $menuname = (!empty($_POST["menuname"])) ? $_POST["menuname"] : "";
            $link = (!empty($_POST["link"])) ? $_POST["link"] : "";
            $sql = "UPDATE plcms_performance SET menuname = \"$menuname\", link = \"$link\" WHERE id=".$id.";";
            mysqli_query($conn, $sql);         
            break;
        case "":
            break;
...
<?php
    $sql = "SELECT * FROM `plcms_performance`  \n"
    . "ORDER BY `plcms_performance`.`orders` ASC;";
    $result = mysqli_query($conn, $sql);
...
?>
      ...
      <div class="modal fade" id="exampleModal<?=$row["id"];?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
              <div class="modal-content" action="performance-manage.php">
...
<div align='center'>
  <a href="logout5.php">ออกจากระบบ</a>
</div>
</body>
</html>
